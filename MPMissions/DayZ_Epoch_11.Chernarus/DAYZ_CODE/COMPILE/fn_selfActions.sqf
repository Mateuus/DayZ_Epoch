...
if(DZE_SELFBLOOD_ENABLE) then {
		diag_log text format["loading SelfBlood addon..."];
		...
		if((speed player <= 1) && hasBagItem && _canDo) then {
			if (s_player_selfBloodbag < 0) then {
				s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"EPOCHMODS\Bloodbag\player_selfbloodbag.sqf","",5,false,true,"", ""];
			};
		} else {
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
		 if (s_player_maintain_area < 0) then {
		  	s_player_maintain_area = player addAction [format["<t color='#ff0000'>%1</t>",localize "STR_EPOCH_ACTIONS_MAINTAREA"], "DAYZ_CODE\COMPILE\maintain_area.sqf", "maintain", 5, false];
		 	s_player_maintain_area_preview = player addAction [format["<t color='#ff0000'>%1</t>",localize "STR_EPOCH_ACTIONS_MAINTPREV"], "DAYZ_CODE\COMPILE\maintain_area.sqf", "preview", 5, false];
		 };
	} else {
	...
	if (_player_studybody) then {
		if (s_player_studybody < 0) then {
				s_player_studybody = player addAction [("<t color=""#FF0000"">"+("Check Wallet") + "</t>"), "DAYZ_CODE\ACTIONS\check_wallet.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
	...
	if(_typeOfCursorTarget in ZSC_MoneyStorage && (player distance _cursorTarget < 5)) then {
		if (s_bank_dialog < 0) then {
				s_bank_dialog = player addAction ["Money Storage", "DAYZ_CODE\ACTIONS\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];	
		};
	} else {
	...
	if( _isVehicle && !_isMan &&_isAlive && !_isMan && !locked _cursorTarget && !(_cursorTarget isKindOf "Bicycle") && (player distance _cursorTarget < 5)) then {		
		if (s_bank_dialog2 < 0) then {
			s_bank_dialog2 = player addAction ["Money Storage", "DAYZ_CODE\ACTIONS\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};			
	} else {		
	...
	if(_typeOfCursorTarget in Bank_Object and (player distance _cursorTarget < 3)) then {		
		if (s_bank_dialog3 < 0) then {
			s_bank_dialog3 = player addAction ["Bank ATM", "DAYZ_CODE\ACTIONS\atm_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	if (s_bank_dialog4 < 0) then {
		s_bank_dialog4 = player addAction ["Transfer Money", "EPOCHMODS\Transfer\transfer_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
	};		
	} else {		
	...
	if((_banker == 1) and (player distance _cursorTarget < 3)) then {		
			if (s_bank_dialog5 < 0) then {
				s_bank_dialog5 = player addAction ["Mr. Teller", "DAYZ_CODE\ACTIONS\atm_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
			};
		if (s_bank_dialog6 < 0) then {
			s_bank_dialog6 = player addAction ["Transfer Money", "EPOCHMODS\Transfer\transfer_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};				
		} else {		
	...
	if(_typeOfCursorTarget in Fast_Med  and (player distance _cursorTarget < 3)) then {		
		if (s_fast_med_dialog1 < 0) then {
			s_fast_med_dialog1 = player addAction ["<t color='#0059FF'>Pay for treatment</t>", "EpochMods\Money\Medical\FastMed.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	} else {		
	...
	if (_isMan and _isAlive and !_isZombie and !_isAnimal and !(_traderType in serverTraders)) then {
		if (s_givemoney_dialog < 0) then {
			s_givemoney_dialog = player addAction [format["Give Money to %1", (name _cursorTarget)], "DAYZ_CODE\ACTIONS\give_player_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	} else {
	...
	if (_isMan && !_isPZombie && _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if(_humanity_logic) then {
				...
				//Advanced Trading
				_buyV = player addAction ["<t color='#0059FF'>Advanced Trading</t>", "EpochMods\Money\AdvancedTrading\ATinit.sqf",(_traderMenu select 0), 999, true, false, "",""];
