...
server_cleanupGroups = {
	...
	{
		if ((count (units _x) == 0) && (_x != grpNull) && !(_x getVariable["SAR_protect",false])) then {
			deleteGroup _x;
		};
		sleep 0.001;
	} count allGroups;
...
server_checkHackers = {
	...
	if (!((isNil "_x") || {(isNull _x)})) then {
		if(vehicle _x != _x && !(vehicle _x in PVDZE_serverObjectMonitor) && (isPlayer _x)  && !((typeOf vehicle _x) in DZE_safeVehicle) && !(_x getVariable["SAR_protect",false]) && (vehicle _x getVariable ["Sarge",0] != 1)  && (vehicle _x getVariable ["Mission",0] != 1)&& (vehicle _x getVariable ["MalSar",0] != 1)) then {
